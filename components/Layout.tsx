...
import { AnimatedLogo } from './animated-logo'  // Add this import at the top of the file

interface LayoutProps {
  children: React.ReactNode;
  hideHeader?: boolean;
  hideFooter?: boolean;
  isAICreatorPage?: boolean;  // Add this new prop
}
...
export default function Layout({ children, hideHeader = false, hideFooter = false, isAICreatorPage = false }: LayoutProps) { 
  ...
  return (
    <div className={`relative min-h-screen flex flex-col md:flex-row ${theme === 'dark' ? 'bg-gray-900 text-white' : ''}`}>
      ...
      {!hideHeader && !isAICreatorPage && (
        /* Beta Version Indicator and User Button */
      ...
      {/* Sidebar */}
      <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:block md:w-14 backdrop-blur-sm p-1 flex flex-col items-center border-r ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="flex flex-col h-full w-full">
          <nav className="space-y-2 py-2">
            <Link href="/">
              <Button variant="ghost" size="icon" title="Home" className="h-9 w-9">
                <PlusCircle className={`h-5 w-5 ${theme === 'dark' ? 'text-white' : 'text-gray-600'}`} />
              </Button>
            </Link>
            <Button variant="ghost" size="icon" title="My Creations" className="h-9 w-9">
              <ImageIcon className={`h-5 w-5 ${theme === 'dark' ? 'text-white' : 'text-gray-600'}`} />
            </Button>
            <Button variant="ghost" size="icon" title="Settings" className="h-9 w-9">
              <Settings className={`h-5 w-5 ${theme === 'dark' ? 'text-white' : 'text-gray-600'}`} />
            </Button>
            <Button variant="ghost" size="icon" title="Help" className="h-9 w-9">
